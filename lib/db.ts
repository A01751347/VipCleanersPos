...
// Agregar esta función a tu lib/db.ts

// Función mejorada para transferir reservación a orden
...
// Agregar estas funciones a tu lib/db.ts
...
// Función mejorada para obtener orden por código (que funcione con el tracking)
export async function getOrderByCodeEnhanced(orderCode: string) {
  try {
...
export async function createOrderFromReservation(
  ...
  try {
    ...
    console.log('Empleado validado:', empleadoExists[0]);
    ...
    console.log('Creando orden con datos validados:', {
      codigoOrden,
      clienteId,
      empleadoId: empleadoExists[0].empleado_id,
      reservacionId,
      fechaEntregaEstimada
    });
    ...
    const result = await executeQuery<any>({
      ...
      values: [
        codigoOrden,
        clienteId,
        empleadoExists[0].empleado_id, // Usar el ID validado
        reservacionId,
        fechaEntregaEstimada,
        notas
      ]
    ...
    console.log('Historial de estados creado exitosamente');
