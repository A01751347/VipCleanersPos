        ...
        <div className="text-center lg:text-left">
          ...
          <AnimateOnScroll type="fade-up" delay={0.8}>
            <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center lg:justify-start">
              {/* Botón principal de reserva (flujo normal) */}
              <BookingButton
              ...
              {/* Botón alternativo: WhatsApp directo */}
              <a
                href="https://wa.me/5512985667?text=Hola%20quiero%20reservar%20una%20limpieza"
                target="_blank"
...
const BadgeLine: React.FC = () => (
  <div className="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/5 px-4 py-1.5">
    <Sparkles size={18} className="text-[color:var(--brand-primary,#78f3d3)]" aria-hidden />
    ...
    <div className="flex -space-x-2">
      {['/assets/LOGO_VIPS.svg','/assets/LOGO_VIPS.svg','/assets/LOGO_VIPS.svg','/assets/LOGO_VIPS.svg','/assets/LOGO_VIPS.svg'].map((src, i) => (
        <Image
          ...
          width={36}
          height={36}
          className="h-9 w-9 rounded-full ring-2 ring-[rgb(17,24,39)] object-contain bg-white"
        />
      ))}
    </div>
    <div className="text-left">
      <div className="flex items-center text-[color:var(--brand-primary,#78f3d3)]">
        {Array.from({ length: 5 }).map((_, i) => (
          <Star key={i} size={14} aria-hidden fill="currentColor" />
        ))}
